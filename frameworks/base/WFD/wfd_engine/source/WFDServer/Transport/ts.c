...
#if defined(RW_THREAD)
	unsigned long long  mPrevInputCounter;
#endif
					   ...
					   TS_CTX_S* ctx)
{
	...
	else
	{
		...
		{
			data_len = data_len + lHdcp_Data_len; 
		}
#endif
...
{ 
	...
	if (  ((ctx->wfd_idx % 7) == 6)  || (ctx->end_marker == 1) ) 
	{
        ...
        if (fstream) {
            ...
            fwrite(ctx->buf0_ptr+RTP_HEADER_SIZE,1,(TS_PACKET_SIZE * (ctx->wfd_idx + 1)),fstream);
        }
        else {
                                                  ...
                                                  //int 		  f_sEnd)
{
	...
	int befCTX_hdcp = 0;
#if defined(RW_THREAD)
      unsigned long long tempInputCounter = 0;
#endif
	...
	if (ctx->es_type == ES_AUDIO) {
		...
		encryptedData = g_aOutputbuffer;
            ...
            if (ctx->hdcpEnabled)
            {
                tempInputCounter = ctx->mInputCounter;
                ctx->mInputCounter = ctx->mPrevInputCounter;
            }
#endif
...

{
	...
	if (err) return 1;
#ifdef TS_WRITE
	err = write_pmt1(id,pmt_pid, codecType);
#else
...
#ifdef TS_WRITE
		     uint32_t    pmt_pid, int codecType)
#else
		     uint32_t    pmt_pid)
#endif
...
#ifdef SWITCH_TRANSPORT
extern int change_TS_RTPTransport(int aType, unsigned aPort) {
	int ret = 0;
	ret = ChangeRTP_RTPTransport(aType, aPort);
	return ret;
}
#endif
...
#if defined(RW_THREAD)
extern int encrytVideoFrame(void 	*id,
                                              byte        data[],
                                              int         dataAddrType,
                                              uint32_t    data_len,
                                              void * output)
{
    ...
    ctx_hdcp = ctx->hdcpEnabled;

    PSIDebug("encrytVideoFrame ++");
    ...
    if (ctx_hdcp)
    {
        ...
        ctx->mPrevInputCounter = ctx->mInputCounter;
        int ret = EncryptFrame(ctx->hdcp_ctx, data, dataAddrType, 
            data_len, ctx->mStrmCounter, &(ctx->mInputCounter), output);
        //ctx->mPrevInputCounter = ctx->mInputCounter;
        if (ret < 0)
        {
            PSIDebug("EncryptFrame failed:%d", ret);
